...
import net.helium.util.Category;
...
public class ClickGUI extends GuiScreen {
    ...
    public void handleMoustInput() throws IOException {
    ...
    protected void keyTyped(final char typedChar, final int keyCode) throws IOException {
        Config.saveConfig();
        if (keyCode == 1 || keyCode == AppleClient.clickGui.getKeycode()) {
    ...
    private void handle(int mouseX, int mouseY, final int key, final float partialTicks, final Handle handle) {
        final int toggled = AppleClient.clickGui.getColor().getRGB();
        final float scale = 2.0f / this.mc.gameSettings.guiScale;
        final int prevScale = this.mc.gameSettings.guiScale;
        GL11.glPushMatrix();
        if (this.mc.gameSettings.guiScale > 2 && !AppleClient.clickGui.scaleGui.isEnabled()) {
        ...
        if (handle == Handle.DRAW) {
            int blur = 0;
            final String selected = AppleClient.clickGui.blur.getSelected();
            switch (selected) {
                case "Low": {
                    blur = 4;
                    break;
        ...
        for (final Panel p : ClickGUI.panels) {
            ...
            y += moduleHeight * (p.prevScrolling + (p.scrolling - p.prevScrolling));
            final List<Module> list = Module.getModulesByCategory(p.category).stream().sorted(Comparator.comparingDouble(module -> Fonts.robotoMediumBold.getStringWidth(module.name))).collect(Collectors.toList());
            Collections.reverse((java.util.List<?>) list);
            for (final Module module2 : list) {
                if (module2.isDevOnly() && !AppleClient.devMode) {
                    continue;
                }
                Label_0942: {
                    switch (handle) {
            ...
            if (p.category == Module.Category.KEYBINDS) {
                switch (handle) {
                    case CLICK: {
                        ...
                        AppleClient.modules.add(keybind);
    ...
    private int getTotalSettingsCount(final Panel panel) {
        ...
        for (final Module module : Module.getModulesByCategory(panel.category)) {
            if (module.isDevOnly() && !AppleClient.devMode) {
                continue;
            }
            ++count;
            if (!module.extended) {
